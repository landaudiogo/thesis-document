...
\begin{algorithm}[h]
    ...
    \While{true}{ 
        ...
        measurement.partitionSizes $\gets$ admin.describeLogDirs(topics)\; 
        ...
        \If{(measurementQueue.first.timestamp - measurement.last.timestamp) > 30s}{ 
            ...
            producer.produce("monitor.writeSpeed", measurementSpeed)\;
            \While{(measurement.timestamp - measurementQueue.first.timestamp) > 30s}{
...
\begin{algorithm}[h]
    ...
    \Input{\\ 
        ...
        mapTopicTable - Map that indicates which table a message from a topic
            is inserted into 
    } 
...
From Table \ref{tab:consumer_testing_conditions}, test 1 has the consumer
...
Figure \ref{fig:consumer_capacity} demonstrates how the consumer is capable of
...
From Figure \ref{fig:consumer_capacity}, the Consumer's capacity is set to $2
